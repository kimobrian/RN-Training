...
const AnimatedHeader = ({ title, nativeScrollY }) => {
  if (nativeScrollY) {
    ...
    const headerBarStyles = { transform: [{ scale: titleScale }, { translateY: titleTranslateY }]};
...
const styles = {
  header_container: { ...Platform.select({ ios: { zIndex: 1 }}) },// only applied to iOS, for some reason the cards is laid on top of the header when scrolling
  header: {
    position: "absolute",
